...
**Backwards compatibility.** No VRL-language changes. Runtime `==` continues to cross variants, so migrating a producer does not change user-visible equality. Rust consumers that distinguish variants will observe the new discriminant; each producer PR should note that in the changelog.
...
The refinement invariant: `string` implies `bytes`. Constructors and mutators keep it directly, because callers query the result without going through canonicalize. `From<&Value> for Kind` maps `Value::String` to `Kind::string()` and `Value::Bytes` to `Kind::bytes()`. Existing `is_bytes()` / `contains_bytes()` keep their meaning -- they check the `bytes` flag -- so they still admit both variants. Any other Kind recognition that treats `bytes` as the full byte-like set must be refinement-aware: `Kind::bytes()` includes `Kind::string()`, not vice versa, and a refined union must not be widened back to unrefined bytes. `is_only_string()` is true iff the kind is exactly the refined string.
...
A refinement survives iff every branch that contributes to the base flag also carries the refinement. That applies to any widening that admits unrefined bytes, not only `Kind::merge`. String concat follows the same rule: both `is_only_string()` → `Kind::string()`; any other string-like combination → `Kind::bytes()`. Repeat preserves the operand's refinement. Comparisons still require only `is_bytes()`.
