...
- `parse_apache_log` / `parse_aws_alb_log` / `parse_csv` / `parse_glog` / `parse_klog` / `parse_key_value` / `parse_nginx_log` / `parse_user_agent` (return objects/arrays whose string fields are parsed from already-validated UTF-8 input).
