...
pub trait Function: Send + Sync {
    ...
    // NEW: Required documentation methods
    fn category(&self) -> &'static str;
    fn description(&self) -> &'static str;

    // NEW: Optional documentation methods (with defaults)
...
- Extend `Function` trait with new documentation methods:
  - Add `category()` and `description()` as required methods
  - Add `notices()` and `deprecated()` with default implementations
- Provide initial stub implementations for all ~150 stdlib functions:
  - Functions can return empty strings for `category()` and `description()` initially
...
**Initial rollout:**
1. Extend `Function` trait with new methods
2. Add stub implementations (empty strings for category/description) to all functions
3. Entire codebase compiles immediately
...
1. **Breaking change to Function trait**: Adding required methods breaks all existing implementations
2. **Build complexity**: Adds another build step and tool to maintain
3. **Initial stub work**: All ~150 functions need stub implementations added (can be automated)
4. **Documentation burden**: Contributors must implement new trait methods for new functions
5. **Validation complexity**: Need tooling to validate categories, track coverage, etc.
6. **Coordination with Vector**: Vector must implement JSON→CUE transformation
...
**Decision**: Rejected - trait methods are simpler, more idiomatic, and enable incremental improvement.
...
**Decision**: Rejected - fragile parsing and type duplication. Hybrid approach extracts types from code.
...
**Security**: Attributes are compile-time metadata only; no code execution; standard code review applies.
...
1. Annotate functions incrementally
2. Generate docs and compare to existing manual docs
3. Target: >90% exact matches validates extraction logic
4. Discrepancies reveal either bugs or documentation improvements
