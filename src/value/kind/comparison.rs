...
impl Kind {
    ...
    pub fn intersects(&self, other: &Self) -> bool {
        ...
        // other side's element kind rules out any common value. If the arrays
        // are disjoint, fall through so that other shared kinds (e.g. object)
        // can still produce an intersection.
...
mod tests {
    ...
    fn test_intersects_arrays() {
        ...
        for (title, TestCase { lhs, rhs, want }) in HashMap::from([
            ...
            (
                "object | array<bytes> vs object | [integer]: shared object kind still intersects",
                TestCase {
                    lhs: Kind::object(BTreeMap::default())
                        .or_array(Collection::from_unknown(Kind::bytes())),
                    rhs: Kind::object(BTreeMap::default()).or_array(Collection::from(
