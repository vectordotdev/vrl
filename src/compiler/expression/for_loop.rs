    ...
    fn resolve_array(&self, arr: Vec<Value>, ctx: &mut Context) -> Resolved {
        match &self.pattern {
            ForPattern::Single(val_node) => {
                let first_ident =
                    (!is_wildcard(val_node.inner())).then(|| val_node.inner().clone());
                let items = arr.into_iter().map(LoopItem::Single);
                self.resolve_items(ctx, first_ident, None, items)
            }
            ForPattern::KeyValue(key_node, val_node) => {
                let first_ident =
                    (!is_wildcard(key_node.inner())).then(|| key_node.inner().clone());
                let second_ident =
                    (!is_wildcard(val_node.inner())).then(|| val_node.inner().clone());
                ...
                for (idx, item) in arr.into_iter().enumerate() {
                    let idx_i64 = i64::try_from(idx).map_err(|_| ExpressionError::Error {
                        ...
                        labels: vec![Label::primary("array index overflow", self.span)],
                        notes: vec![],
                    })?;
                    items.push(LoopItem::KeyValue(Value::Integer(idx_i64), item));
                }
    ...
    fn resolve_object(&self, map: ObjectMap, ctx: &mut Context) -> Resolved {
        match &self.pattern {
            ForPattern::KeyValue(key_node, val_node) => {
                ...
                let first_ident = has_first.then(|| key_node.inner().clone());
                let second_ident =
                    (!is_wildcard(val_node.inner())).then(|| val_node.inner().clone());

                let items = map.into_iter().map(|(key, val)| {
                    let key_val = if has_first {
                        Value::from(key)
                    } else {
                        Value::Null
                    };
                    LoopItem::KeyValue(key_val, val)
                });
                self.resolve_items(ctx, first_ident, second_ident, items)
            }
            ForPattern::Single(_) => {
...
impl<'c, 'b> ForScopeGuard<'c, 'b> {
    ...
    pub fn new(
        ...
        let second_ident = if first_ident == second_ident {
            None
        } else {
            second_ident
        };
...
impl Drop for ForScopeGuard<'_, '_> {
    fn drop(&mut self) {
        if let Some(ident) = &self.first_ident {
...
mod tests {
    ...
    fn test_scope_guard_in_place_mutation() {
        ...
        let mut guard = ForScopeGuard::new(&mut ctx, Some(x_id.clone()), Some(y_id.clone()));
    ...
    fn test_scope_guard_restores_shadowed_on_drop() {
        ...
        {
            let mut ctx = test_ctx(&mut target, &mut state, &tz);
            let mut guard = ForScopeGuard::new(&mut ctx, Some(x_id.clone()), None);
    ...
    fn test_break_and_continue_diagnostics_code_and_labels() {
        ...
        assert_eq!(
            break_err.code(),
            codes::CompilerCode::LoopControlOutsideLoop as usize
        );
        assert_eq!(
            continue_err.code(),
            codes::CompilerCode::LoopControlOutsideLoop as usize
    ...
    fn test_scope_guard_has_methods() {
        ...
        let guard = ForScopeGuard::new(&mut ctx, Some(Ident::new("k")), None);
    ...
    fn test_scope_guard_selective_setters() {
        ...
        // 1. Both present
        {
            let mut guard = ForScopeGuard::new(&mut ctx, Some(k_id.clone()), Some(v_id.clone()));
            assert!(guard.has_first());
            assert!(guard.has_second());
        ...
        {
            let mut guard_first = ForScopeGuard::new(&mut ctx, Some(k_id.clone()), None);
        ...
        {
            let mut guard_second = ForScopeGuard::new(&mut ctx, None, Some(v_id.clone()));
