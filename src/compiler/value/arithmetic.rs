...
impl VrlValueArithmetic for Value {
    ...
    fn try_mul(self, rhs: Self) -> Result<Self, ValueError> {
        ...
        let value = match self {
            ...
            Value::Float(lhv) => {
                let rhs = rhs.try_into_notnan_f64().map_err(|_| err())?;
                lhv.mul(rhs).into()
            }
    ...
    fn try_rem(self, rhs: Self) -> Result<Self, ValueError> {
        ...
        let value = match self {
            Value::Integer(lhs) if rhs.is_float() => {
                Value::from_f64_or_zero(lhs as f64 % rhs.try_float()?)
            }
            Value::Integer(lhs) => {
                let rhs = rhs.try_into_i64().map_err(|_| err())?;
                i64::wrapping_rem(lhs, rhs).into()
            }
            Value::Float(lhs) => {
                let rhs = rhs.try_into_notnan_f64().map_err(|_| err())?;
                lhs.rem(rhs).into()
