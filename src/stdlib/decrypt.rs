...
fn decrypt(ciphertext: Value, algorithm: &str, key: Value, iv: Value) -> ValueResult {
...
impl FunctionExpression for DecryptFn {
    fn resolve(&self, ctx: &mut Context) -> Resolved {
        let ciphertext = crate::resolve_value!(self.ciphertext.resolve(ctx));
        ...
        decrypt(ciphertext, algorithm.as_str(), key, iv).map(EvaluationOutcome::Value)
    }
